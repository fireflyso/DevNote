...
for pipe_id in pipe_list:
    ...
    res = client.execute("SELECT time, in_bps, out_bps from flow_snmp.flow_data_first_local01 WHERE pipe_id  = '{}' and time > '2021-06-01 12:00:00' and time < '2021-07-01 12:00:00'".format(pipe_id))
    count += len(res)

print("ck 新数据表 查询用时：{} s, 共计查询结果 {} 条".format((datetime.now() - start_time).seconds, count))
